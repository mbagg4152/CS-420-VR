...
public class Hover : MonoBehaviour {
    ...
    void Start() {
        mainCam = GameObject.Find("UICam").GetComponent<Camera>(); // separate camera for UI elements
    ...
    void OnMouseOver(){
        ...
        if (dist <= 12) {
            label.text = this.tag;
            qmark.enabled = true;
            if (Input.GetKey(KeyCode.JoystickButton15) || Input.GetKey(KeyCode.Mouse0)) { // allow right trigger or mouse click
                infoPanel.enabled = true;
                plantInfo(this.tag); // display the information about the plant
            }
        }
            
    }

    void OnMouseExit(){ // update UI elements when focus changes
    ...
    void plantInfo(String tag){
        switch(tag){
            ...
            case "Boabab":
             ...
             case "Rubber Plant":
                nameText.text = "Scientific name:\nFicus elastica\n\nCommon name(s):\nRubber Plant";
                infoText.text = "This plant is actually part of the fig genus and cannot be made for rubber. Similar to its relative the figs you'll find at the store, they need the help of a fig wasp to become pollinated.";
                break;
            default:
                nameText.text = "name";
